...
*/
     ...
     if (digitalRead(overswitch)==LOW){
       if(onetime==1){
        ...
        if(inTime + interval < currentTime && inTime + interval + 1500UL > currentTime){
...
{
 {
    server.on("/", []() {
      ...
      content += "<p>";
      content += st;
      content += "</p><form method='get' action='setting'><label>SSID: </label><input name='ssid' length=32><input name='pass' length=32><input type='submit'></form>";
      content += "</html>";
